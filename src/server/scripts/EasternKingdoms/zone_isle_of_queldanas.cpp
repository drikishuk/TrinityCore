...
public:
    ...
    struct npc_thalorien_dawnseeker_AI : public ScriptedAI
    {
        npc_thalorien_dawnseeker_AI(Creature* creature) : ScriptedAI(creature), _summons(me)
        {
        }
            ...
            while (uint32 eventId = _events.ExecuteEvent())
            {
                ...
                case EVENT_THALORIEN_GO:
                    if (Creature* thalorien = ObjectAccessor::GetCreature(*me, _thalorienGUID))
                    {
                        ...
                        thalorien->HandleEmoteCommand(EMOTE_STATE_READY2H);
    ...
    {
        return new npc_thalorien_dawnseeker_AI(creature);
    }
};
