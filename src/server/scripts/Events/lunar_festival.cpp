...
public:
    ...
    struct npc_firework_AI : public ScriptedAI
    {
        npc_firework_AI(Creature* creature) : ScriptedAI(creature) { }
        ...
        uint32 GetFireworkGameObjectId()
        {
            ...
            if (spellInfo && spellInfo->Effects[EFFECT_0].Effect == SPELL_EFFECT_SUMMON_OBJECT_WILD)
                return spellInfo->Effects[EFFECT_0].MiscValue;
        ...
        void Reset() override
        {
            ...
            if (isCluster())
            {
                ...
                for (uint8 i = 0; i < 4; i++)
                    me->SummonGameObject(GetFireworkGameObjectId(), me->GetPositionX() + (i % 2 == 0 ? displacement : -displacement), me->GetPositionY() + (i > 1 ? displacement : -displacement), me->GetPositionZ() + 4.0f, me->GetOrientation(), QuaternionData(), 1000);
            }
            else
    ...
    {
        return new npc_firework_AI(creature);
    }
};
...
public:
    ...
    struct npc_omen_AI : public ScriptedAI
    {
        npc_omen_AI(Creature* creature) : ScriptedAI(creature)
        {
    ...
    CreatureAI* GetAI(Creature* creature) const override
    {
        return new npc_omen_AI(creature);
    }
};
...
public:
    ...
    struct npc_giant_spotlight_AI : public ScriptedAI
    {
        npc_giant_spotlight_AI(Creature* creature) : ScriptedAI(creature) { }
    ...
    CreatureAI* GetAI(Creature* creature) const override
    {
        return new npc_giant_spotlight_AI(creature);
    }
};
...
class spell_lunar_festival_elune_candle : public SpellScriptLoader
{
public:
    spell_lunar_festival_elune_candle() : SpellScriptLoader("spell_lunar_festival_elune_candle") { }

    class spell_lunar_festival_elune_candle_SpellScript : public SpellScript
    {
        ...
        {
            OnEffectHitTarget.Register(&spell_lunar_festival_elune_candle_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_DUMMY);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_lunar_festival_elune_candle_SpellScript();
    }
};
...
void AddSC_event_lunar_festival()
{
    ...
    new spell_lunar_festival_elune_candle();
}
